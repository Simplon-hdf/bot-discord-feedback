...
export async function editQuestionText(interaction: ModalSubmitInteraction) {
    try {
        // Récupérer l'index de la question à partir de l'ID du modal
        const questionIndex = parseInt(interaction.customId.replace("editQuestionTextModal_", ""));
        ...
        const newText = interaction.fields.getTextInputValue(`questionText_${questionIndex}`);
        question.content = newText;
        ...
        await interaction.editReply({
            content: `Question ${questionIndex + 1} : ${newText}`,
        ...
        const followUp = await interaction.followUp({
            content: "Le texte de la question a été modifié avec succès !",
            flags: MessageFlags.Ephemeral
        });
